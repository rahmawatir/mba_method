...
if (isset($_POST['submit'])) {
    ...
    if (empty($_POST['min_support']) || empty($_POST['min_confidence'])) {
        $can_process = false;
        ?>
        <script> location.replace("?menu=proses_eclat&pesan_error=Min Support dan Min Confidence harus diisi");</script>
        <?php
    }
    if(!is_numeric($_POST['min_support']) || !is_numeric($_POST['min_confidence'])){
        $can_process = false;
        ?>
        <script> location.replace("?menu=proses_eclat&pesan_error=Min Support dan Min Confidence harus diisi angka");</script>
        <?php
    }
    ...
    if($can_process){
        ...
        if(isset($_POST['id_process'])){
            ...
            // $query = $db_object->insert_record("transaksi_sort", $field_value);
