...
if ($num_rows > 0) {
    $rows = $db->db_fetch_array($result);
        ...
        $_SESSION['gadde_id'] = $rows['id']; // mengisi session
        $_SESSION['gadde_username'] = $rows['username'];
        $_SESSION['gadde_level'] = $rows['level'];

        $level_name = ($_SESSION['gadde_level']==1)?"admin":"kepala";
        $_SESSION['gadde_level_name'] = $level_name;
        $_SESSION['gadde_key'] = sha1(date("Y-m-d H:i:s") . $rows['id']);
        $_SESSION['gadde_last_login'] = date("d-m-Y H:i:s");
